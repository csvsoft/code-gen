...
case class DoobiekafKaMsgRepository(xa: Transactor[Task]) extends KafkaMsgRepository with Log {
  ...
  def getLatest(topic: String, n: Int): AppTask[List[KafkaMessage]] = {
    logger.info(s"Getting latest kafka message for topic: $topic")
    SQL
      .getLatest(n)
      .to[List]
      .transact(xa)
...
object DoobiekafKaMsgRepository {
  ...
  object SQL {
    ...
    def create(msg: KafkaMessage): Update0 =
      sql""" INSERT INTO KAFKA_MESSAGE(
           |  TOPIC,
           |  BIZ_KEY,
           |  MSG_HEADERS,
           |  MSG_KEY,
    ...
    def delete(id: KafkaMessageId): Update0 =
      sql"""
    ...
    def update(msg: KafkaMessage): Update0 =
      sql"""
